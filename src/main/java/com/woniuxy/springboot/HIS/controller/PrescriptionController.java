...
public class PrescriptionController {
	...
	public String submitPrescription(HttpServletRequest req) {
		try {
			Allprescriptions prescriptionInfo = (Allprescriptions) req.getSession().getAttribute("prescription");
			if (prescriptionInfo.getPrescriptions().isEmpty()) {
				req.setAttribute("msg", "无数据");
				return "liugong/prescription";
			}
	...
	public String selectPrescription(HttpServletRequest req, String pid, Date cfdate) {
		try {
			...
			List<Allprescriptions> listP = prescriptionService.selectPrescriptionByPidAndCfdate(npid, cfdate);
			if (listP.isEmpty()) {
	...
	public String retrunPrescription(HttpServletRequest req, String pid, Date cfdate) {
		try {
			...
			List<Allprescriptions> listP = prescriptionService.selectPrescriptionByPidAndCfdate2(npid, cfdate);
			if (listP.isEmpty()) {
				req.setAttribute("msg", "未查到信息");
			}
			req.setAttribute("Allprescriptions", listP);
	...
	// 查询删除session中的记录
	@RequestMapping("/delectOnePresciption")
	public String delectOnePresciption(HttpServletRequest req, String mname) {
		
	Allprescriptions prescriptionInfo = (Allprescriptions) req.getSession().getAttribute("prescription");
	List<Prescription> prescriptions = prescriptionInfo.getPrescriptions();
	for (int i = 0; i < prescriptions.size(); i++) {
		if(prescriptions.get(i).getMedicine().getMname().equals(mname)) {
			BigDecimal b1 = new BigDecimal(prescriptionInfo.getCfcount()+"");
			BigDecimal b2 = new BigDecimal(prescriptions.get(i).getMcount()+"");
			prescriptionInfo.setCfcount(b1.subtract(b2).doubleValue());
	...
	public String updatePresciption(HttpServletRequest req, String mname) {
		
		Allprescriptions prescriptionInfo = (Allprescriptions) req.getSession().getAttribute("prescription");
		List<Prescription> prescriptions = prescriptionInfo.getPrescriptions();
		for (int i = 0; i < prescriptions.size(); i++) {
			if(prescriptions.get(i).getMedicine().getMname().equals(mname)) {
				req.setAttribute("onePrescription", prescriptions.get(i));
				break;
			}
		}
		return "liugong/updatePrescription";
		
	}
	// 更新数据
	@PostMapping("/updatePrescription")
	public String updatePresciptionsub(HttpServletRequest req, String mname,String mnum,String mway) {
		int nnum = 0;
		try {
			nnum = Integer.parseInt(mnum);
			if(nnum<=0) {
		...
		Allprescriptions prescriptionInfo =
				(Allprescriptions) req.getSession().getAttribute("prescription");
		List<Prescription> prescriptions = prescriptionInfo.getPrescriptions();
		for (int i = 0; i < prescriptions.size(); i++) {
			if(prescriptions.get(i).getMedicine().getMname().equals(mname)) {
				prescriptions.get(i).setMnum(nnum);
				prescriptions.get(i).setMway(mway);
				BigDecimal b1 = new BigDecimal(prescriptionInfo.getCfcount()+"");
				BigDecimal b2 = new BigDecimal(prescriptions.get(i).getMcount()+"");
				BigDecimal b3 = new BigDecimal(nnum+"").multiply(
						new BigDecimal(prescriptions.get(i).getMedicine().getMprice()+""));
				prescriptions.get(i).setMcount(b3.doubleValue());
				prescriptionInfo.setCfcount(b1.subtract(b2).add(b3).doubleValue());
				break;
			}
		}
		return "liugong/addPrescription";
